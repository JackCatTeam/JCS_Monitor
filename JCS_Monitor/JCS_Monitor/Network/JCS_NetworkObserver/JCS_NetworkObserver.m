...
NSString *const kJCS_NetworkObserverEnabledStateChangedNotification = @"kJCS_NetworkObserverEnabledStateChangedNotification";
static NSString *const kJCS_NetworkObserverEnabledDefaultsKey = @"com.JCS_.JCS_NetworkObserver.enableOnLaunch";

typedef void (^NSURLSessionAsyncCompletion)(id fileURLOrData, NSURLResponse *response, NSError *error);

@interface JCS_InternalRequestState : NSObject
...
@implementation JCS_InternalRequestState

@end

@interface JCS_NetworkObserver (NSURLConnectionHelpers)
...
@interface JCS_NetworkObserver (NSURLSessionTaskHelpers)
...
@interface JCS_NetworkObserver ()

@property (nonatomic) NSMutableDictionary<NSString *, JCS_InternalRequestState *> *requestStatesForRequestIDs;
@property (nonatomic) dispatch_queue_t queue;

@end

@implementation JCS_NetworkObserver
...
+ (void)setEnabled:(BOOL)enabled
{
    ...
    [[NSUserDefaults standardUserDefaults] setBool:enabled forKey:kJCS_NetworkObserverEnabledDefaultsKey];
    ...
    if (previouslyEnabled != enabled) {
        [NSNotificationCenter.defaultCenter postNotificationName:kJCS_NetworkObserverEnabledStateChangedNotification object:self];
    }
}

+ (BOOL)isEnabled
{
    return [[[NSUserDefaults standardUserDefaults] objectForKey:kJCS_NetworkObserverEnabledDefaultsKey] boolValue];
}
...
{
    static JCS_NetworkObserver *sharedObserver = nil;
...
+ (void)sniffWithoutDuplicationForObject:(NSObject *)object selector:(SEL)selector sniffingBlock:(void (^)(void))sniffingBlock originalImplementationBlock:(void (^)(void))originalImplementationBlock
{
    ...
    // See https://github.com/Flipboard/JCS_/issues/61 for an example.
    if (!object) {
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (numClasses > 0) {
            ...
            for (NSInteger classIndex = 0; classIndex < numClasses; ++classIndex) {
                ...
                if (class == [JCS_NetworkObserver class]) {
                    continue;
                }
...
+ (void)injectIntoNSURLConnectionCancel
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        SEL swizzledSelector = [JCS_NetworkUtility swizzledSelectorForSelector:selector];
        Method originalCancel = class_getInstanceMethod(class, selector);

        void (^swizzleBlock)(NSURLConnection *) = ^(NSURLConnection *slf) {
            [[JCS_NetworkObserver sharedObserver] connectionWillCancel:slf];
...
+ (void)injectIntoNSURLSessionTaskResume
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        void (^swizzleBlock)(NSURLSessionTask *) = ^(NSURLSessionTask *slf) {
            ...
            [[JCS_NetworkObserver sharedObserver] URLSessionTaskWillResume:slf];
            ((void(*)(id, SEL))objc_msgSend)(slf, swizzledSelector);
        };
...
+ (void)injectIntoNSURLConnectionAsynchronousClassMethod
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        void (^asyncSwizzleBlock)(Class, NSURLRequest *, NSOperationQueue *, NSURLConnectionAsyncCompletion) = ^(Class slf, NSURLRequest *request, NSOperationQueue *queue, NSURLConnectionAsyncCompletion completion) {
            if ([JCS_NetworkObserver isEnabled]) {
                NSString *requestID = [self nextRequestID];
                [[JCS_NetworkRecorder defaultRecorder] recordRequestWillBeSentWithRequestID:requestID request:request redirectResponse:nil];
                NSString *mechanism = [self mechanismFromClassMethod:selector onClass:class];
                [[JCS_NetworkRecorder defaultRecorder] recordMechanism:mechanism forRequestID:requestID];
                NSURLConnectionAsyncCompletion completionWrapper = ^(NSURLResponse *response, NSData *data, NSError *connectionError) {
                    [[JCS_NetworkRecorder defaultRecorder] recordResponseReceivedWithRequestID:requestID response:response];
                    [[JCS_NetworkRecorder defaultRecorder] recordDataReceivedWithRequestID:requestID dataLength:data.length];
                    if (connectionError) {
                        [[JCS_NetworkRecorder defaultRecorder] recordLoadingFailedWithRequestID:requestID error:connectionError];
                    } else {
                        [[JCS_NetworkRecorder defaultRecorder] recordLoadingFinishedWithRequestID:requestID responseBody:data];
                    }
        ...
        [JCS_NetworkUtility replaceImplementationOfKnownSelector:selector onClass:class withBlock:asyncSwizzleBlock swizzledSelector:swizzledSelector];
    });
}
    ...
    dispatch_once(&onceToken, ^{
        ...
        NSData *(^syncSwizzleBlock)(Class, NSURLRequest *, NSURLResponse **, NSError **) = ^NSData *(Class slf, NSURLRequest *request, NSURLResponse **response, NSError **error) {
            ...
            if ([JCS_NetworkObserver isEnabled]) {
                ...
                [[JCS_NetworkRecorder defaultRecorder] recordResponseReceivedWithRequestID:requestID response:temporaryResponse];
                [[JCS_NetworkRecorder defaultRecorder] recordDataReceivedWithRequestID:requestID dataLength:data.length];
                if (temporaryError) {
                    [[JCS_NetworkRecorder defaultRecorder] recordLoadingFailedWithRequestID:requestID error:temporaryError];
                } else {
                    [[JCS_NetworkRecorder defaultRecorder] recordLoadingFinishedWithRequestID:requestID responseBody:data];
                }
                if (error) {
        ...
        [JCS_NetworkUtility replaceImplementationOfKnownSelector:selector onClass:class withBlock:syncSwizzleBlock swizzledSelector:swizzledSelector];
    });
}
...
+ (void)injectIntoNSURLSessionAsyncDataAndDownloadTaskMethods
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (int selectorIndex = 0; selectorIndex < numSelectors; selectorIndex++) {
            ...
            if ([JCS_NetworkUtility instanceRespondsButDoesNotImplementSelector:selector class:class]) {
            ...
            NSURLSessionTask *(^asyncDataOrDownloadSwizzleBlock)(Class, id, NSURLSessionAsyncCompletion) = ^NSURLSessionTask *(Class slf, id argument, NSURLSessionAsyncCompletion completion) {
                ...
                if ([JCS_NetworkObserver isEnabled] && completion) {
                    NSString *requestID = [self nextRequestID];
                    NSString *mechanism = [self mechanismFromClassMethod:selector onClass:class];
            ...
            [JCS_NetworkUtility replaceImplementationOfKnownSelector:selector onClass:class withBlock:asyncDataOrDownloadSwizzleBlock swizzledSelector:swizzledSelector];
        }
    });
}
...
+ (void)injectIntoNSURLSessionAsyncUploadTaskMethods
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (int selectorIndex = 0; selectorIndex < numSelectors; selectorIndex++) {
            ...
            [JCS_NetworkUtility replaceImplementationOfKnownSelector:selector onClass:class withBlock:asyncUploadTaskSwizzleBlock swizzledSelector:swizzledSelector];
        }
    });
}
...
+ (NSURLSessionAsyncCompletion)asyncCompletionWrapperForRequestID:(NSString *)requestID mechanism:(NSString *)mechanism completion:(NSURLSessionAsyncCompletion)completion
{
    NSURLSessionAsyncCompletion completionWrapper = ^(id fileURLOrData, NSURLResponse *response, NSError *error) {
        ...
        [[JCS_NetworkRecorder defaultRecorder] recordDataReceivedWithRequestID:requestID dataLength:data.length];
        if (error) {
            [[JCS_NetworkRecorder defaultRecorder] recordLoadingFailedWithRequestID:requestID error:error];
        } else {
            [[JCS_NetworkRecorder defaultRecorder] recordLoadingFinishedWithRequestID:requestID responseBody:data];
        }
...
+ (void)injectWillSendRequestIntoDelegateClass:(Class)cls
{
    ...
    NSURLConnectionWillSendRequestBlock undefinedBlock = ^NSURLRequest *(id <NSURLConnectionDelegate> slf, NSURLConnection *connection, NSURLRequest *request, NSURLResponse *response) {
        [[JCS_NetworkObserver sharedObserver] connection:connection willSendRequest:request redirectResponse:response delegate:slf];
        return request;
    };
    ...
    [JCS_NetworkUtility replaceImplementationOfSelector:selector withSelector:swizzledSelector forClass:cls withMethodDescription:methodDescription implementationBlock:implementationBlock undefinedBlock:undefinedBlock];
}
...
+ (void)injectDidReceiveResponseIntoDelegateClass:(Class)cls
{
    ...
    NSURLConnectionDidReceiveResponseBlock undefinedBlock = ^(id <NSURLConnectionDelegate> slf, NSURLConnection *connection, NSURLResponse *response) {
        [[JCS_NetworkObserver sharedObserver] connection:connection didReceiveResponse:response delegate:slf];
    };
...
+ (void)injectDidReceiveDataIntoDelegateClass:(Class)cls
{
    ...
    NSURLConnectionDidReceiveDataBlock undefinedBlock = ^(id <NSURLConnectionDelegate> slf, NSURLConnection *connection, NSData *data) {
        [[JCS_NetworkObserver sharedObserver] connection:connection didReceiveData:data delegate:slf];
    };
...
+ (void)injectDidFinishLoadingIntoDelegateClass:(Class)cls
{
    ...
    NSURLConnectionDidFinishLoadingBlock undefinedBlock = ^(id <NSURLConnectionDelegate> slf, NSURLConnection *connection) {
        [[JCS_NetworkObserver sharedObserver] connectionDidFinishLoading:connection delegate:slf];
    };
...
+ (void)injectDidFailWithErrorIntoDelegateClass:(Class)cls
{
    ...
    NSURLConnectionDidFailWithErrorBlock undefinedBlock = ^(id <NSURLConnectionDelegate> slf, NSURLConnection *connection, NSError *error) {
        [[JCS_NetworkObserver sharedObserver] connection:connection didFailWithError:error delegate:slf];
    };
...
+ (void)injectTaskWillPerformHTTPRedirectionIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionWillPerformHTTPRedirectionBlock undefinedBlock = ^(id <NSURLSessionTaskDelegate> slf, NSURLSession *session, NSURLSessionTask *task, NSHTTPURLResponse *response, NSURLRequest *newRequest, void(^completionHandler)(NSURLRequest *)) {
        [[JCS_NetworkObserver sharedObserver] URLSession:session task:task willPerformHTTPRedirection:response newRequest:newRequest completionHandler:completionHandler delegate:slf];
...
+ (void)injectTaskDidReceiveDataIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionDidReceiveDataBlock undefinedBlock = ^(id <NSURLSessionDataDelegate> slf, NSURLSession *session, NSURLSessionDataTask *dataTask, NSData *data) {
        [[JCS_NetworkObserver sharedObserver] URLSession:session dataTask:dataTask didReceiveData:data delegate:slf];
    };
...
+ (void)injectDataTaskDidBecomeDownloadTaskIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionDidBecomeDownloadTaskBlock undefinedBlock = ^(id <NSURLSessionDataDelegate> slf, NSURLSession *session, NSURLSessionDataTask *dataTask, NSURLSessionDownloadTask *downloadTask) {
        [[JCS_NetworkObserver sharedObserver] URLSession:session dataTask:dataTask didBecomeDownloadTask:downloadTask delegate:slf];
    };
...
+ (void)injectTaskDidReceiveResponseIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionDidReceiveResponseBlock undefinedBlock = ^(id <NSURLSessionDelegate> slf, NSURLSession *session, NSURLSessionDataTask *dataTask, NSURLResponse *response, void(^completionHandler)(NSURLSessionResponseDisposition disposition)) {
        [[JCS_NetworkObserver sharedObserver] URLSession:session dataTask:dataTask didReceiveResponse:response completionHandler:completionHandler delegate:slf];
...
+ (void)injectTaskDidCompleteWithErrorIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionTaskDidCompleteWithErrorBlock undefinedBlock = ^(id <NSURLSessionTaskDelegate> slf, NSURLSession *session, NSURLSessionTask *task, NSError *error) {
        [[JCS_NetworkObserver sharedObserver] URLSession:session task:task didCompleteWithError:error delegate:slf];
    };
...
+ (void)injectDownloadTaskDidFinishDownloadingIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionDownloadTaskDidFinishDownloadingBlock undefinedBlock = ^(id <NSURLSessionTaskDelegate> slf, NSURLSession *session, NSURLSessionDownloadTask *task, NSURL *location) {
        ...
        [[JCS_NetworkObserver sharedObserver] URLSession:session task:task didFinishDownloadingToURL:location data:data delegate:slf];
    };
...
+ (void)injectDownloadTaskDidWriteDataIntoDelegateClass:(Class)cls
{
    ...
    NSURLSessionDownloadTaskDidWriteDataBlock undefinedBlock = ^(id <NSURLSessionTaskDelegate> slf, NSURLSession *session, NSURLSessionDownloadTask *task, int64_t bytesWritten, int64_t totalBytesWritten, int64_t totalBytesExpectedToWrite) {
        [[JCS_NetworkObserver sharedObserver] URLSession:session downloadTask:task didWriteData:bytesWritten totalBytesWritten:totalBytesWritten totalBytesExpectedToWrite:totalBytesExpectedToWrite delegate:slf];
    };
...
static char const * const kJCS_RequestIDKey = "kJCS_RequestIDKey";

+ (NSString *)requestIDForConnectionOrTask:(id)connectionOrTask
{
    NSString *requestID = objc_getAssociatedObject(connectionOrTask, kJCS_RequestIDKey);
    if (!requestID) {
...
{
    objc_setAssociatedObject(connectionOrTask, kJCS_RequestIDKey, requestID, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (id)init
{
    ...
    if (self) {
        ...
        self.queue = dispatch_queue_create("com.JCS_.JCS_NetworkObserver", DISPATCH_QUEUE_SERIAL);
    }
    return self;
}
...
- (JCS_InternalRequestState *)requestStateForRequestID:(NSString *)requestID
{
    JCS_InternalRequestState *requestState = self.requestStatesForRequestIDs[requestID];
    if (!requestState) {
        requestState = [JCS_InternalRequestState new];
...
@implementation JCS_NetworkObserver (NSURLConnectionHelpers)

- (void)connection:(NSURLConnection *)connection willSendRequest:(NSURLRequest *)request redirectResponse:(NSURLResponse *)response delegate:(id<NSURLConnectionDelegate>)delegate
{
    [self performBlock:^{
        NSString *requestID = [[self class] requestIDForConnectionOrTask:connection];
        JCS_InternalRequestState *requestState = [self requestStateForRequestID:requestID];
        requestState.request = request;
        [[JCS_NetworkRecorder defaultRecorder] recordRequestWillBeSentWithRequestID:requestID request:request redirectResponse:response];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection delegate:(id<NSURLConnectionDelegate>)delegate
{
    [self performBlock:^{
        NSString *requestID = [[self class] requestIDForConnectionOrTask:connection];
        JCS_InternalRequestState *requestState = [self requestStateForRequestID:requestID];
        [[JCS_NetworkRecorder defaultRecorder] recordLoadingFinishedWithRequestID:requestID responseBody:requestState.dataAccumulator];
...
@implementation JCS_NetworkObserver (NSURLSessionTaskHelpers)
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition disposition))completionHandler delegate:(id<NSURLSessionDelegate>)delegate
{
    [self performBlock:^{
        ...
        [[JCS_NetworkRecorder defaultRecorder] recordMechanism:requestMechanism forRequestID:requestID];
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didWriteData:(int64_t)bytesWritten totalBytesWritten:(int64_t)totalBytesWritten totalBytesExpectedToWrite:(int64_t)totalBytesExpectedToWrite delegate:(id<NSURLSessionDelegate>)delegate
{
    [self performBlock:^{
        ...
        if (!requestState.dataAccumulator) {
            ...
            [[JCS_NetworkRecorder defaultRecorder] recordResponseReceivedWithRequestID:requestID response:downloadTask.response];

            NSString *requestMechanism = [NSString stringWithFormat:@"NSURLSessionDownloadTask (delegate: %@)", [delegate class]];
            [[JCS_NetworkRecorder defaultRecorder] recordMechanism:requestMechanism forRequestID:requestID];
        }

        [[JCS_NetworkRecorder defaultRecorder] recordDataReceivedWithRequestID:requestID dataLength:bytesWritten];
    }];
}
...
- (void)URLSessionTaskWillResume:(NSURLSessionTask *)task
{
    ...
    [self performBlock:^{
        ...
        if (!requestState.request) {
            ...
            [[JCS_NetworkRecorder defaultRecorder] recordRequestWillBeSentWithRequestID:requestID request:task.currentRequest redirectResponse:nil];
        }
    }];
}
